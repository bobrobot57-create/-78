...
Для наплыва одновременных проверок лицензий можно задать:
- `DB_POOL_SIZE=40` — размер пула (по умолчанию 40)
- `DB_CONCURRENT_LIMIT=32` — макс. одновременных запросов к БД (семафор, по умолчанию 32)
- `THREAD_POOL_SIZE=32` — потоков для asyncio.to_thread (по умолчанию 32)
